...
public class UserService extends UtilityFunctions {
    ...
    public ResponseEntity<CustomResponseStatus> renewOTP(String oldOtpString, HttpServletRequest request) {
        final Otp otp = otpService.generateNewOtp(oldOtpString);
        if (otp != null) {
            User user = otp.getUser();
            otpService.sendNewUserVerificationEmail(user, otp.getOtpString(), getAppUrl(request), request.getLocale());
